//Modified from a program by Aditya Prakash(CG TA 2015 IITB)
...
// {
  GLuint LoadShader(const std::string vertex_shader_file, const std::string fragment_shader_file){
        ...
        shaderList.push_back(LoadShaderGL(GL_VERTEX_SHADER, vertex_shader_file));
        shaderList.push_back(LoadShaderGL(GL_FRAGMENT_SHADER, fragment_shader_file));
        return CreateProgramGL(shaderList);
  }
